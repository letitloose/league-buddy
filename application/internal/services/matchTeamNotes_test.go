...
func TestSaveNoteRoundTripAndResubmit(t *testing.T) {
	...
	if err := noteService.SaveNote(matchID, 1, &MatchTeamNoteForm{PlayerOfMatchID: playerID, Notes: "Sam ran the show"}); err != nil {
		t.Fatal(err)
	}
	if err := noteService.SaveNote(matchID, 1, &MatchTeamNoteForm{Notes: "actually a team effort"}); err != nil {
		t.Fatal(err)
	}
